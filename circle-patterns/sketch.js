...
const sketch = () => {
  ...
  for (var l = 0; l < columns; l++) {
      for (var d = 0; d < rows; d++) {
          resetMatrix();
          translate((l + 1) * offset + dia * l + dia * (l + 1), (d + 1) * offset + dia * d + dia * (d + 1));
          var rn = int(random(10, 30));
          for (var a = 0; a < 360; a += 30) {
              push();
              rotate(radians(a));
              for (var r = 0; r < 180; r += rn) {
                  line(Math.sin(radians(r)) * dia, Math.cos(radians(r)) * dia, Math.sin(radians(-r)) * dia, Math.cos(radians(-r)) * dia);
              }
              pop();
          }
          noFill();
